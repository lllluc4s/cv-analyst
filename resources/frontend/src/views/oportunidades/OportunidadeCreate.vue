...
const handleSubmit = async () => {
  try {
    ...
    await oportunidadesService.create(form.value)
    router.push('/admin/oportunidades')
  } catch (err: any) {
    if (err.response?.status === 422) {
      errors.value = err.response.data.errors
    } else {
      error.value = 'Erro ao criar oportunidade. Verifique se o backend está rodando.'
    }
    console.error(err)
  } finally {
    loading.value = false
...
<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900">Nova Oportunidade</h1>
      <p class="text-gray-600 mt-1">Crie uma nova vaga de emprego</p>
    </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="flex justify-between pt-6 border-t">
          ...
          >
            ...
            <span v-else>Criar Oportunidade</span>
          </button>
        </div>
      </form>
    </div>
